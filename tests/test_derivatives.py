...
@pytest.fixture
def shape(xdim=20, ydim=20, zdim=20):
...
def test_stencil_derivative(grid, shape, SymbolType, dim):
    ...
    i = dim(grid)  # Known pytest issue: cannot promptly use fixtures with parametrize
...
def test_derivatives_space(grid, derivative, dim, order):
    ...
    dim = dim(grid)  # Known pytest issue: cannot promptly use fixtures with parametrize
